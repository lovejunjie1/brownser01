...
class textureAssetManager(assetManager_BaseClass):
    ...
    def downloadModelFn(self):
        ...
        if itm.parent():
            ...
            itemNode = itm
        '''
        infoPath = self.serverPath + '\\asset\\' + self.departmentName + '\\' + self.nowTreePage + '\\' + itemNode.text(0) + '\\infomation.json'
        ...
        innerMsg = '[ TEXTURE ][ download ] copy the model from model path to texture path'
    ...
    def oprbtn_uploadFn(self, data):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            ...
            filename = self.projectName + '_' + tyStr + '_' + changeTabItm.text(0) + self.departmentName +'.ma'
            ...
            innerMsg = '[ TEXTURE ][ upload ] ' + ' <br> updateTime : ' + time.strftime("%Y-%m-%d %H:%M:%S",
    ...
    def confirmFn(self, cData):
        ...
        progress = QtWidgets.QProgressDialog("uploading......", \
                                             'cancel', 0, len(updateDict) + 10, self)
        progress.setWindowModality(Qt.WindowModal)
        ...
        innerMsg = '[ ' + self.departmentName + ' ][ confirm ] ' + ' <br> confirmTime : ' + time.strftime("%Y-%m-%d %H:%M:%S",                                                                               time.time())) + '<br>confirm by ' + userName
        htmlMsg = self.HTML_createMsg(innerMsg, userName, '', side=1, initPath=self.initPath)
        self.HTML_addMsg(historypath, htmlMsg)
        #history data saved.
