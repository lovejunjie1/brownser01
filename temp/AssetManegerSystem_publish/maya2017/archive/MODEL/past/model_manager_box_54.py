...
# follow the rig codes to add some fcuntions and fix 9 bugs.


#import AssetManegerSystem_pathConfig as pathConfig
...
# path='D:\\AssetManegerSystem'
#path = getScriptPath()
#if not path in sys.path: sys.path.append(path)
#if not path + '\\MODEL' in sys.path: sys.path.append(path + '\\MODEL')
#if not path + '\\bin' in sys.path: sys.path.append(path + '\\bin')
#if not path + '\\data' in sys.path: sys.path.append(path + '\\data')

import maya.cmds as cmds
import maya.OpenMayaUI as OpenMayaUI
#import userLoginUI02 as userLoginUI
...
#Mstyle.setIconPath(path + '\\icon')
from PySide.QtWebKit import QWebView, QWebPage
#import UploadFileUI05 as modelUploadUI
#import ConfirmUI11 as modelConfirmUI
#import docWriter_reader3 as docWriter_reader
#import docWriter_writter7 as docWriter_writter
...
class ModelManagerBoxUI(QtGui.QDialog, MayaQWidgetDockableMixin, chooseIcon):
    ...
    nowPicPath = ''
    keepLogin = 'Login'
    localSetPath = 'D:\\Program Files\\figo\\ModelManaBox_Local\\'
    nowTreePage = 'Char'

    def __init__(self, parent=getMayaWindow()):
        self.cleanOpenWindow()
        super(ModelManagerBoxUI, self).__init__(parent=parent)
    ...
    def readNote(self):
        ...
        thewidget = docWriter_reader.writerReaderUI()
        thewidget.move(0, 0)
    ...
    def createGeneralPage(self):
        ...
        preBtn.setIcon(QtGui.QIcon(getScriptPath() + '\\icon\\prePage_red.png'))
        ...
        nextBtn.setIcon(QtGui.QIcon(getScriptPath() + '\\icon\\nextPage_red.png'))
    ...
    def oprbtn_openUploadFn(self):
        self.rigUp = modelUploadUI.submitTool()
        self.rigUp.setParent(self)
        self.rigUp.getNotifyFromTxt(self.initPath + '\\MODEL\\notify.txt')
        self.rigUp.createUI()
        self.rigUp.Op_Ui()
    ...
    def oprbtn_confirmFn(self):
        if self.treeWid.currentItem():
            ...
            confirm = modelConfirmUI.Rig_confirm_TOOL()
