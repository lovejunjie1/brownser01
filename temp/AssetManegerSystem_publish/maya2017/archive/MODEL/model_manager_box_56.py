...
class waitingBar(QtWidgets.QDialog):
...
class dragToolListWidget(QtWidgets.QTreeWidget):
    onPress = False
    iArray = []
    isDrag = False
    RigPath = ''
...
class assetManager_ModelBoxUI(QtWidgets.QDialog, MayaQWidgetDockableMixin, chooseIcon):
    titleName = 'ModelManagerBoxUI'
    widgetHeight = 750
    widgetWidth = 600
    initPath = ''
    UIitems = {}
    ...
    def __init__(self, parent=None):
        ...
        super(assetManager_ModelBoxUI, self).__init__(parent=parent)
    ...
    def loadConfig(self):
        ...
        self.localSetPath = pathDict['localSetPath'] + '\\ModelManaBox_Local\\'
    ...
    def createTitleBar(self):
        qmb = QtWidgets.QMenuBar(self)

        self.fileMn = QtWidgets.QMenu(self.keepLogin, qmb)
        ...
        shMn = QtWidgets.QMenu('Layout', qmb)
        qmb.addMenu(shMn)
        self.shMn2act = QtWidgets.QAction("FileList", shMn, checkable=True)
        self.shMn2act.setChecked(True)
        shMn.addAction(self.shMn2act)
        self.shMn3act = QtWidgets.QAction("MoreInfo", shMn, checkable=True)
        self.shMn3act.setChecked(True)
        shMn.addAction(self.shMn3act)
        shMn.addSeparator()

        toolMn = QtWidgets.QMenu('Tool', qmb)
        qmb.addMenu(toolMn)
        tool1act = toolMn.addAction("Tool1")
        tool2act = toolMn.addAction("Tool2")

        aboutMn = QtWidgets.QMenu('About', qmb)
        qmb.addMenu(aboutMn)
    ...
    def createTab(self):
        ...
        self.tabWid.setTabPosition(QtWidgets.QTabWidget.West)
        ...
        comReadBtn = QtWidgets.QPushButton('Read')
        ...
        self.comLE = QtWidgets.QListWidget()
        self.comLE.verticalScrollBar().setStyleSheet(Mstyle.QScrollBar())
        self.comLE.setIconSize(QtCore.QSize(48, 48))
        self.comLE.setLayoutMode(QtWidgets.QListWidget.Batched)

        comLineE = QtWidgets.QLineEdit()
        ...
        hisVB = QtWidgets.QVBoxLayout()
        hisVB.setContentsMargins(0, 0, 0, 0)
        hisVB.setSpacing(0)

        blanklabelup = QtWidgets.QLabel()
        blanklabelup.setFixedHeight(24)
        blanklabelbelow = QtWidgets.QLabel()
        ...
        self.conceptHbox = QtWidgets.QGridLayout()
    ...
    def readNote(self):
        ...
        filedata = ''
        noteDir = self.serverPath + '\\asset\\MODEL\\' + self.nowTreePage + '\\' + itemNode.text(
    ...
    def openEidtWindow(self):
        ...
        TEmain.savePath = self.serverPath + '\\asset\\MODEL\\' + self.nowTreePage + '\\' + itemNode.text(0) + '\\note'
        TEmain.initUI()
    ...
    def createGeneralPage(self):
        ...
        chaLab = QtWidgets.QLabel('charactorName')
        ...
        modelGrp = QtWidgets.QGroupBox('concept')
        ...
        infoGrp = QtWidgets.QGroupBox('infomation')
        ...
        labLastMsgTitle = QtWidgets.QLabel('LastMsg : ', msgWid)
        labLastMsgTitle.setStyleSheet(Mstyle.QLabel(fontSize='10px', kw='b'))
        labLastMsg = QtWidgets.QLabel('', msgWid)
        ...
        lab4 = QtWidgets.QLabel('Artist : ', msgWid)
        lab4.setStyleSheet(Mstyle.QLabel(fontSize='10px', kw='b'))
        lab4data = QtWidgets.QLabel('', msgWid)
        ...
        lab5 = QtWidgets.QLabel('Passing : ', msgWid)
        lab5.setStyleSheet(Mstyle.QLabel(fontSize='10px', kw='b'))
        lab5data = QtWidgets.QLabel('none days', msgWid)
        ...
        lab6 = QtWidgets.QLabel('StartDate :     ', msgWid)
        lab6.setStyleSheet(Mstyle.QLabel(fontSize='10px', kw='b'))
        lab6data = QtWidgets.QLabel('', msgWid)
        ...
        lab7 = QtWidgets.QLabel('ConfirmDate : ', msgWid)
        lab7.setStyleSheet(Mstyle.QLabel(fontSize='10px', kw='b'))
        lab7data = QtWidgets.QLabel('', msgWid)
        ...
        lab8 = QtWidgets.QLabel('Machine : ', msgWid)
        lab8.setStyleSheet(Mstyle.QLabel(fontSize='10px', kw='b'))
        lab8data = QtWidgets.QLabel('', msgWid)
        ...
        oprbtn_uploadLow = QtWidgets.QPushButton('upload', msgWid)
        ...
        oprbtn_confirmLow = QtWidgets.QPushButton('Confirm', msgWid)
    ...
    def refrenceModelFn(self):
        ...
        path = self.serverPath + '\\asset\\MODEL\\' + self.nowTreePage + '\\' + itemNode.text(0) + '\\DownloadConcept\\'
    ...
    def oprbtn_openUploadFn(self):
        ...
        self.rigUp.getNotifyFromTxt(self.initPath + '\\maya\\MODEL\\notify.txt')
        self.rigUp.createUI()
        self.rigUp.Op_Ui()
    ...
    def oprbtn_uploadFn(self, list):
        ...
        if list[0] == 'l':
            buttonKey = 'low'
            filename = 'PIG_' + tyStr + '_' + itemNode.text(0) + '_mod_low.ma'
        ...
        if list[0] == 'h':
            buttonKey = 'high'
            filename = 'PIG_' + tyStr + '_' + itemNode.text(0) + '_mod_high.ma'
        ...
        innerMsg = '[ MODEL ][ upload ] ' + buttonKey + ' <br> updateTime : ' + time.strftime("%Y-%m-%d %H:%M:%S",
    ...
    def createTree(self):
        ...
        radHBox = QtWidgets.QHBoxLayout()

        rad1 = QtWidgets.QPushButton('cha')
        ...
        rad2 = QtWidgets.QPushButton('prop')
        ...
        rad3 = QtWidgets.QPushButton('env')
        ...
        self.treeWid.RigPath = self.serverPath + '\\asset\\MODEL'
    ...
    def createTreeList(self, page='Char'):
        allowArray = ['ma']
        folders = self.getFolders(path=(self.serverPath + '\\asset\\MODEL\\' + page))
        ...
        for f in folders:
            ...
            files = self.getFiles(self.serverPath + '\\asset\\MODEL\\' + page + '\\' + f)
            for fl in files:
                flsp = fl.split('.')[-1]
                if flsp in allowArray:
                    citm = QtWidgets.QTreeWidgetItem()
                    citm.setText(0, fl)
    ...
    def copyConceptToModelFn(self):
        ...
        if modeltime == '' or concepttime > modeltime:
            ...
            infoPath = self.serverPath + '\\asset\\MODEL\\' + self.nowTreePage + '\\' + changeTabItm.text(
                0) + '\\infomation.json'
            ...
            innerMsg = '[ MODEL ] copy the concept from concept path to model path'
            htmlMsg = self.HTML_createMsg(innerMsg, userName, '', side=1, initPath=self.initPath)
            self.HTML_addMsg(historypath, htmlMsg)
    ...
    def loadGeneralPageData(self, changeTabItm):
        ...
        filePath = self.serverPath + '\\asset\\MODEL\\' + self.nowTreePage + '\\' + changeTabItm.text(0) + '\\'
        ...
        servDate.setText(conceptStrTime)
        localDate.setText(modelStrTime)
        ...
        if type(changeTabItm) != str:
            ...
            if (os.path.exists(historypath)):
                ...
                self.qweb.setHtml(historydata.read(), QtCore.QUrl(
                    self.serverPath + '\\asset\\MODEL\\' + self.nowTreePage + '\\' + changeTabItm.text(0) + '\\note'))
            else:
                self.qweb.setHtml('', '')
        pass
    ...
    def loadConceptPictureData(self, changeTabItm):
        ...
        self.conceptPath = self.serverPath + '\\asset\\CONCEPT\\' + self.nowTreePage + '\\' + changeTabItm.text(0)
        ...
        if isConceptExt:
            ...
            for i in temps:
                isf = os.path.isfile(self.conceptPath + '\\' + i)
                if isf:
                    pixmap = QtGui.QPixmap()
                    pixmap.load(self.conceptPath + '\\' + i)
                    pixlab = clickLabel()
                    pixlab.setFixedWidth(120)
                    pixlab.setFixedHeight(120)
                    pixlab.setPixmap(pixmap.scaledToWidth(120))
                    pixlab.setToolTip(self.conceptPath + '\\' + i)
    ...
    def loadNoteListData(self):
        ...
        noteDir = self.serverPath + '\\asset\\MODEL\\' + self.nowTreePage + '\\' + itemNode.text(0) + '\\note'
...
# ModelManaBUI=ModelManagerBoxUI()
# ModelManaBUI.Op_Ui()
