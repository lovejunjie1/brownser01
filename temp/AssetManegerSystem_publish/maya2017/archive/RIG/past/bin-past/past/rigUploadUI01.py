...
class Rig_submit_TOOL(QtGui.QWidget):
    ...
    isSubmitPic=False
    uploadSignal=QtCore.Signal(str)
    ...
    def __init__(self):
        ...
        default404=''
        for i in sys.path:
            check=os.path.isfile(i+'/404NotFound.png') 
            if check:
                default404=i+'/404NotFound.png'
    ...
    def saveScenceImage(self,path,isFront=True):
        ...
        number=str(len(dirs))
        fullPath=tempPath+'tempCut'+number+'.png'
        camShape=''
        if isFront:
            mel.eval('dR_DoCmd("viewFront")')
            camShape='frontShape'
        else:
            type = cmds.modelPanel(cmds.getPanel(wf=True), q=True, cam=True)
        ...
        self.saveImageSignal.emit(fullPath)
        print fullPath
        return fullPath
    ...
    def CheckBoxMainFn(self):
        ...
        if False in checkState:
            self.BtnA1.setEnabled(False)
            self.BtnA2.setEnabled(False)
            self.BtnA1.setStyleSheet(Mstyle.btnOffStyleChinese)    
            self.BtnA2.setStyleSheet(Mstyle.btnOffStyleChinese)  
        else:
            self.BtnA1.setEnabled(True)
            self.BtnA2.setEnabled(True)
            self.BtnA1.setStyleSheet(Mstyle.btnOnStyleChinese)    
            self.BtnA2.setStyleSheet(Mstyle.btnOnStyleBChinese) 

        self.checkLab.setText((str(count)+' of '+str(self.checkCount+1)+' items checked'))
    def BtnA1Fn(self):
        chaOrProp = cmds.confirmDialog(title='choose type',message='you want submit a ....?',button=['cha', 'prop','cancel'],defaultButton='cha',cancelButton='cancel',dismissString='cancel')
        if chaOrProp != 'cancel':
            message=self.TEA1.toPlainText()
            message_utf=message.encode("utf-8") 
            message_sys=urllib.quote(message_utf)
            rigSubmitTool.message=message_sys
            
            rigSubmitTool.reviever='mdsh047,mdsh038,mdsh003,brandon'
            rigSubmitTool.run(rigType='high',cop=chaOrProp) 
            
            toPath=rigSubmitTool.serverPath+'/showPic.png'
            projectPath=cmds.workspace(q=1,rd=1)
            fromPath=projectPath+'temp.0001.png'
    ...
    def BtnA2Fn(self):
        ...
        if chaOrProp != 'cancel':
            ...
            rigSubmitTool.reviever='mdsh047,mdsh038,mdsh003,brandon'
            rigSubmitTool.run(rigType='low',cop=chaOrProp) 
