...
class RigManagerBoxUI(QtGui.QDialog ,MayaQWidgetDockableMixin, chooseIcon):
    ...
    def createTab(self):
        ...
        comWriteBtn.clicked.connect(self.openEditWindow)
        comEditBtn.clicked.connect(self.openEditWindow)
        
    def openEditWindow(self):
        sender=self.sender()
        winClass=textEditorMain()
        winClass.setParent(self)
        winClass.saveSignal.connect(self.saveEditWindow)
        if 'write' in sender.text():
            #create new
            winClass.show()
        if 'edit' in sender.text():
            #open the current select item
            pass
            
    def saveEditWindow(self,list):
        print list
    ...
    def treeWidChangedFn(self):
        ...
        htmlMsg=HTML_createMsg(self,'the message',userName,'',side=1,initPath=self.initPath) 
        '''
        #historypath=r'Z:\project\pig\asset\PRODUCE\Char\monkeyking\history.html'
        #historydata=open(historypath)
        
        #self.qweb.setHtml(historydata.read(),QtCore.QUrl("d:\\"))
