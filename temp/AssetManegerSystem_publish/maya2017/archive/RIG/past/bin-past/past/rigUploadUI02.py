...
class Rig_submit_TOOL(QtGui.QWidget):
    ...
    isSubmitPic=False
    uploadHighSignal=QtCore.Signal(str)
    uploadLowSignal=QtCore.Signal(str)
    ...
    def BtnA1Fn(self):
        ...
        self.uploadHighSignal.emit([self.sceenRtn,message])

    def BtnA2Fn(self):
        message=self.TEA1.toPlainText()
        #message_utf=message.encode("utf-8") 
        self.uploadLowSignal.emit([self.sceenRtn,message])
