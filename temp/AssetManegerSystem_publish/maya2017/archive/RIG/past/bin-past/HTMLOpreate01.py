class HTML_opreate():
    initPath='D:\\figo_tool_box'
    historyPath=''
    def HTML_CreateEmptyStyle(self):
        style=(
            '<style type="text/css">  \n' 
        ...
        reportnew = open(self.historyPath, 'w')  
    ...
    def HTML_createMsg(self,contect,user,pictures,side=1): 
        pic='' 
        if pictures:
            for p in pictures[:3]:
        ...
        if not usePic:
            usePic=self.initPath+'\\icon\\lifeStyle18.png'
        if side>0:
