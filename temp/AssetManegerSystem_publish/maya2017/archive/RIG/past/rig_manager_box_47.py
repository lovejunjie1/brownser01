...
path = getScriptPath()
# sys.path.insert(0,"PIL????C:\\python\\PIL")
if not path in sys.path: sys.path.insert(0, path)
if not path + '\\RIG' in sys.path: sys.path.append(path + '\\RIG')
...
import UploadFileUI05 as rigUploadUI
import ConfirmUI11 as rigConfirmUI
...
class RigManagerBoxUI(QtGui.QDialog, MayaQWidgetDockableMixin, chooseIcon):
    ...
    def oprbtn_openUploadFn(self):
        ...
        self.rigUp = rigUploadUI.submitTool()
        self.rigUp.setParent(self)
        self.rigUp.getNotifyFromTxt(self.initPath + '\\RIG\\notify.txt')
        self.rigUp.createUI()
        self.rigUp.Op_Ui()
