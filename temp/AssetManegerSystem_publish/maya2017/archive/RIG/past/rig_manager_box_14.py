    ...
    def copyModelToRigFn(self):
        ...
        if 'low' in sender.toolTip():
            if self.nowLowModel:
                ...
                print 'to'
                print self.serverPath+'\\'+self.nowTreePage
                #os.system ("xcopy /s /c /r /i /y %s %s" % (Spath, Lpath))
                    ...
                    if type(c)==QtGui.QGroupBox:
                        ...
                        if c.title()=='model':
                            ...
                            if os.path.exists(rigPath):
                                ...
                                lowPathLocal=rigPath+'\\'+lowName
                                hignPathLocal=rigPath+'\\'+highName
                                if os.path.exists(lowPathLocal):
                                    lFileLoc=lowPathLocal
                                    self.nowLowModelLocal=lFileLoc
                                if os.path.exists(hignPathLocal):
                                    hFileLoc=hignPathLocal
                                    self.nowHighModelLocal=hFileLoc
