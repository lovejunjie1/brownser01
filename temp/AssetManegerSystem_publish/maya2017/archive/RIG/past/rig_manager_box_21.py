...
import bin.rigUploadUI04 as rigUploadUI
...
class RigManagerBoxUI(MayaQWidgetDockableMixin, QtGui.QDialog ,chooseIcon):
    ...
    def createGeneralPage(self):
        ...
        oprbtn_uploadLow.clicked.connect(self.oprbtn_openUploadLowFn)
        #oprbtn_uploadHigh.clicked.connect(self.oprbtn_openUploadHighFn)
        oprbtn_confirmLow.clicked.connect(self.oprbtn_confirmLowFn)
        oprbtn_confirmHigh.clicked.connect(self.oprbtn_confirmHighFn)
        return msgWid
        
    def oprbtn_openUploadLowFn(self):
        #rigUp=rigUploadUI.Rig_submit_TOOL()
        rigUp=Rig_submit_TOOL()
        rigUp.uploadLowSignal.connect(self.tempFn)
        rigUp.uploadHighSignal.connect(self.tempFn)
        rigUp.Op_Ui()
        
    def tempFn(self,list):
        print 'active'
        print list
        
    def oprbtn_uploadLowFn(self):
        pass
        ...
        filename='PIG_'+tyStr+'_'+self.treeWid.currentItem ().text(0)+'rig_low.ma'
    ...
    def oprbtn_uploadHighFn(self):
        ...
        if self.nowTreePage=='Char':
            ...
            tyStr=self.nowTreePage.lower() 
        filename='PIG_'+tyStr+'_'+self.treeWid.currentItem ().text(0)+'rig_high.ma'
    ...
    def oprbtn_confirmLowFn(self):
        print 'confirm low'
    def oprbtn_confirmHighFn(self):
        print 'confirm high'
    ...
    def loadGeneralPageData(self,itm):
        ...
        if isExt:
            ...
            emptyDict={'artist':['none'],'passing':'','startDate':['notStart'],'confirmDate':['notConfirm'],'machine':['empty']}
