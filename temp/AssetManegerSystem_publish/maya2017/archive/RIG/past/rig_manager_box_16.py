...
class RigManagerBoxUI(MayaQWidgetDockableMixin, QtGui.QDialog ,chooseIcon):
    ...
    def copyModelToRigFn(self):
        ...
        if self.fileMn.title().lower()!='login':
            dict['artist'].append(self.fileMn.title())
        else:
            dict['artist'].append(getpass.getuser())
        
        sDate=time.strftime("%m-%d %H:%M:%S %Y", time.localtime(time.time()))  
        dict['startDate'].append(sDate)
        dict['machine'] = getpass.getuser()
            ...
            if type(c)==QtGui.QGroupBox:
                ...
                if c.title()=='infomation':
                    ...
                    if isExt:
                        ...
                        emptyDict={'artist':['none'],'passing':'','startDate':['notStart'],'confirmDate':['notConfirm'],'machine':''}
