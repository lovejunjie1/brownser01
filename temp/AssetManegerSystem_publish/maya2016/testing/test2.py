    ...
    def createGif(self, dataPath):
        ...
        for count, i in enumerate(dataArray):
            ...
            self.createPlayblastUI(i,
                                   ...
                                   imageSize=picSize,
                                   isReplace=True)
        outputMovie = self.__output + '/' + self.__camName + '.gif'
        ffmpegCommand =(self.__translaterPath
                        + ' -y -f image2 -r '
                        + str(self.__fps[3:])
                        + ' -i '
...
ans2 = OMP.outputPlayblast('sq02_sc04_sh0030_100f_209f_fps24Shape')
print ans2


CTM = sequenceToMov()
CTM.setCameraName('sq02_sc04_sh0030_100f_209f_fps24')
CTM.setAudio(ans2[2][0], ans2[2][1])
CTM.createMovie(ans2[0])
CTM.createGif(ans2[0])
