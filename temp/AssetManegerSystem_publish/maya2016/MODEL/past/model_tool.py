...
class modelAssetManager(assetManager_BaseClass):
    def extraInit(self):
        self.departmentName = 'MODEL'
        self.projectName = 'PIG'

    def howToUseFn(self):
        thePath = os.path.realpath(__file__)
        dirPath = os.path.dirname(thePath)
        docPath = dirPath + '\\help.doc'
    ...
    def loadGeneralPageData(self):
        ...
        # load infomation
        infoPath = self.serverPath + '\\asset\\' + self.departmentName + '\\' + self.nowTreePage + '\\' + changeTabItm.text(
            0) + '\\infomation.json'
        timeNodePath = self.serverPath + '\\asset\\' + self.departmentName + '\\' + self.nowTreePage + '\\' + changeTabItm.text(
            0) + '\\timeNode.json'
        ...
        timeNodeExt = os.path.exists(timeNodePath)
        if timeNodeExt:
            dict = self.loadJson(timeNodePath)
            if dict['startDate']:
    ...
    def downloadConceptFn(self):
        ...
        if not os.path.exists(infoPath):
            emptyDate = {'startDate': [], 'confirmDate': [], 'artist': [], 'machine': []}
            self.saveJson(emptyDate, infoPath)
    ...
    def oprbtn_uploadFn(self, data):
        ...
        if reply == QtGui.QMessageBox.Yes:
            ...
            dataDict = {"artist": [userName], "machine": [getpass.getuser()], "uploadDate": [nowdate],
                        "message": [data[-1]]}
            self.saveJson(dataDict, uploadPath + 'infomation.json')
    ...
    def confirmFn(self, cData):
        ...
        for uf in updateFiles:
            ...
            if uf[0] != '.':
                if spUf[-1] != 'ma':
                    # print 'copy : ' + updatePathFile
                    # print 'to   : ' + mainPathFile
        ...
        innerMsg = '[ ' + self.departmentName + ' ][ confirm ] ' + ' <br> confirmTime : ' + time.strftime(
            "%Y-%m-%d %H:%M:%S",
            time.localtime(
                time.time())) + '<br>confirm by ' + userName
        htmlMsg = self.HTML_createMsg(innerMsg, userName, '', side=1, initPath=self.initPath)
        self.HTML_addMsg(historypath, htmlMsg)
