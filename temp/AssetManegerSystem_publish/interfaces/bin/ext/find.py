...
class docWritter_Find(qw.QDialog):
    ...
    def initUI(self):
        ...
        findButton = qw.QPushButton("Find",self)
        findButton.clicked.connect(self.find)

        # Button to replace the last finding
        replaceButton = qw.QPushButton("Replace",self)
        replaceButton.clicked.connect(self.replace)

        # Button to remove all findings
        allButton = qw.QPushButton("Replace all",self)
        allButton.clicked.connect(self.replaceAll)

        # Normal mode - radio button
        self.normalRadio = qw.QRadioButton("Normal",self)

        # Regular Expression Mode - radio button
        regexRadio = qw.QRadioButton("RegEx",self)

        # The field into which to type the query
        self.findField = qw.QTextEdit(self)
        self.findField.resize(250,50)

        # The field into which to type the text to replace the
        # queried text
        self.replaceField = qw.QTextEdit(self)
    ...
    def find(self):
        ...
        if self.normalRadio.isChecked():
            ...
            if self.lastStart >= 0:
                ...
                self.parent.text.moveCursor(qw.QTextCursor.End)
    ...
    def moveCursor(self,start,end):
        ...
        cursor.movePosition(qw.QTextCursor.Right,qw.QTextCursor.KeepAnchor,end - start)
