...
class assetManager_DragTreeWidget(qw.QTreeWidget):
    onPress = False
    iArray = []
    RigPath = ''
    localPath = ''
    ...
    def setPathDict(self,theDict):  
        self.pathList = theDict
    ...
    def showContextMenu(self, pos):
        ...
        if self.currentItem().parent():
            actionA = self.contextMenu.addAction(qw.QIcon(iconPath + "drag.png"), u'refrence')
            actionA.triggered.connect(self.actionAFn)
            actionB = self.contextMenu.addAction(qw.QIcon(iconPath + "floder.png"), u'open')
            actionB.triggered.connect(self.actionBFn)

        actionC = self.contextMenu.addAction(qw.QIcon(iconPath + "CommboBoxArrow.png"), u'dowload')
