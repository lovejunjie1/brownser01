...
from interfaces.bin.assetManager_HTML import assetManager_HTML
from interfaces.bin.getMayaWindow import getMayaWindow
from interfaces.bin.assetManager_userLoginUI import assetManager_userLoginUI
from interfaces.bin.assetManager_DragTreeWidget import assetManager_DragTreeWidget
from interfaces.bin.assetManager_writterUI import assetManager_writterUI
from interfaces.bin.assetManager_submitTool import assetManager_submitTool
...
class clickLabel(qw.QLabel):
    def mousePressEvent(self, e):
        if (e.button() == QtCore.Qt.LeftButton):
            os.startfile(self.toolTip())


class assetManager_BaseClass(qw.QDialog, MayaQWidgetDockableMixin,assetManager_HTML):
    titleName = ''
    widgetHeight = 950
    widgetWidth = 600
    initPath = ''
    ...
    def createTitleBar(self):
        ...
        self.MainMenuBar = qw.QMenuBar(self)

        self.fileMn = qw.QMenu(self.keepLogin, self.MainMenuBar)
        ...
        self.projectMn = qw.QMenu('project List', self.MainMenuBar)
        self.MainMenuBar.addMenu(self.projectMn)

        self.projectActionGrp = qw.QActionGroup(self)
        ...
        shMn = qw.QMenu('Layout', self.MainMenuBar)
        self.MainMenuBar.addMenu(shMn)
        self.shMn2act = qw.QAction("FileList", shMn, checkable=True)
        self.shMn2act.setChecked(True)
        shMn.addAction(self.shMn2act)
        self.shMn3act = qw.QAction("MoreInfo", shMn, checkable=True)
        self.shMn3act.setChecked(True)
        shMn.addAction(self.shMn3act)
        shMn.addSeparator()

        aboutMn = qw.QMenu('About', self.MainMenuBar)
        ...
        self.projectActionGrp.triggered.connect(self.projectChangeFn)
    ...
    def projectChangeFn(self,theAct):
        ...
        ans = qw.QMessageBox.information( self, "project Changed",
                                                  ('project changed to ' + str(theAct.text()) + '\nplease restart toolkit'),
                                                  qw.QMessageBox.Yes | qw.QMessageBox.No)
        if qw.QMessageBox.Yes == ans:
    ...
    def createSplitter(self):
        ...
        self.splitter = qw.QSplitter(self)
    ...
    def createTab(self):
        ...
        self.tabWid.setTabPosition(qw.QTabWidget.West)
    ...
    def createGeneralTab(self):
        self.GWid = qw.QWidget()
        self.GWid.setAttribute(QtCore.Qt.WA_StyledBackground)
        self.GWid.setStyleSheet('QWidget{background:rgb(109,109,109);border-radius: 12px}')
        self.GWid.setAttribute(QtCore.Qt.WA_DeleteOnClose)

        self.tabWid.addTab(self.GWid, 'general')

    def createConceptTab(self):
        ConceptWid = qw.QWidget()
        ConceptWid.setAttribute(QtCore.Qt.WA_StyledBackground)
        ConceptWid.setStyleSheet('QWidget{background:rgb(109,109,109);border-radius: 12px}')
        ConceptWid.setAttribute(QtCore.Qt.WA_DeleteOnClose)

        self.conceptHbox = qw.QGridLayout()
    ...
    def createCommonTab(self):
        ...
        comReadBtn = qw.QPushButton('Read')
        ...
        self.comLE = qw.QListWidget()
        self.comLE.verticalScrollBar().setStyleSheet(Mstyle.QScrollBar())
        self.comLE.setIconSize(QtCore.QSize(48, 48))
        self.comLE.setLayoutMode(qw.QListWidget.Batched)

        comLineE = qw.QLineEdit()
        comLineE.setText('search bar')
        comLineE.setAlignment(QtCore.Qt.AlignHCenter)
    ...
    def createTree(self):
        ...
        radHBox = qw.QHBoxLayout()

        rad1 = qw.QPushButton('cha')
        ...
        projectLabel.setAlignment(QtCore.Qt.AlignCenter)
    ...
    def loadNoteListData(self):
        ...
        if os.path.exists(noteDir):
            ...
            for f in folders:
                ...
                linearGradient = qw.QLinearGradient(60, 50, 200, 200)
                linearGradient.setColorAt(0, QtCore.Qt.transparent)
                linearGradient.setColorAt(0.79, QtCore.Qt.transparent)
                linearGradient.setColorAt(0.8, qw.QColor(200, 200, 200))
                linearGradient.setColorAt(0.84, qw.QColor(200, 200, 200))
                linearGradient.setColorAt(0.85, qw.QColor(colorArray[cn][0], colorArray[cn][1], colorArray[cn][2]));
                linearGradient.setColorAt(1.0, qw.QColor(colorArray[cn][0], colorArray[cn][1], colorArray[cn][2]));
                ...
                itm.setIcon(icn)
                itm.setToolTip(f)
                itm.setBackground(qw.QBrush(linearGradient))
                self.comLE.addItem(itm)
                count += 1
                # dir(qw.QListWidgetItem)
                # QListWidgetItem
        pass
