...
if not fileDir in sys.path:sys.path.append(fileDir)
...
class FTextEdit(QtGui.QTextEdit):
    copySignal = QtCore.pyqtSignal(str)
...
class textEditorMain(QtGui.QMainWindow):
    saveSignal = QtCore.pyqtSignal(list)
    initPath = ''
    savePath = ''
    itemName = ''
    filename = ''
    ...
    def __init__(self, parent=None):
        ...
        self.initPath = script.getAssetManagerPath()['icon'] + '\\writer_icon\\'
    ...
    def pasteFn(self):
        ...
        isPic = MD.hasImage()
        if isPic:
            tempPath = 'C:/Program Files/figo/QtDoc_picTemp/'
    ...
    def save(self):
            ...
            if data:
                ...
                for d in data:
                    ...
                    if sAddress == address:
                        ...
                        ndsp = []
                        for fuck in dsp:
                            ndsp.append(str(fuck))
                        newData.append('"'.join(ndsp))
                strData=[]
                for nd in newData:
                    strData.append(str(nd))
                newText = '<img '.join(strData)
        else:
            newText = self.text.toHtml()

        # print newText

        encodeText = str(newText).encode('utf-8')
        # print encodeText
        ...
        simpleInfo = unicode(self.text.toPlainText())
        #unicode(msg)
        ...
        if self.filename:
            ...
            if len(gg) > 1:
                ...
                if data:
                    ...
                    nnewData = []
                    for nd in newData:
                        nnewData.append(str(nd))
                    newText = '<img '.join(nnewData)
            else:
                newText = self.text.toHtml()
    ...
    def cutImage(self):
        if os.path.exists(self.screenDllPath):
            subprocess.call(r'rundll32.exe ' + self.screenDllPath + ' PrScrn')
        else:
    ...
    def insertImage(self):
        ...
        # Get image file name
        filename = \
        QtGui.QFileDialog.getOpenFileName(self, 'Insert image', ".", "Images (*.png *.xpm *.jpg *.bmp *.gif)")[0]
        print filename
        if filename:
